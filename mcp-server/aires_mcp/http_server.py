...
Переменные окружения:
  AIRES_MCP_TOKENS   ``токен1:Имя,токен2:Другое Имя`` — обязательна;
                     без неё сервер не поднимется, иначе база данных оказалась бы
                     открыта всему интернету.
  AIRES_API_URL      адрес API базы данных (как в stdio-режиме).
...
def load_tokens(raw: str | None = None) -> dict[str, str]:
    ...
    интернета, и молчаливое открытие базы данных было бы худшим из возможных
    ...
    if not tokens:
        raise TokenGuardError(
            "AIRES_MCP_TOKENS пуст: HTTP-сервер без токенов открыл бы базу данных всем"
        )
    return tokens
...
class TokenGuard:
    ...
    async def __call__(self, scope: Any, receive: Any, send: Any) -> None:
        ...
        # базе данных, зато позволяет мониторингу видеть, что сервис поднят.
        if path == "/healthz":
