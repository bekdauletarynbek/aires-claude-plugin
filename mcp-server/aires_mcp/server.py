...
def build_server() -> Any:
    ...
    mcp = FastMCP(
        "aires",
        instructions=(
            "База данных научных статей, разобранных под задачи Freedom "
            ...
            "базу данных у инструментов нет — они только читают.\n\n"
            "Как отвечать человеку: источник называйте «база данных» — "
            "слово «корпус» не используйте. Внутреннюю кухню в ответ не "
            "выносите: названия артефактов (IPR, PRD, конспект, план "
            "презентации), статусы обработки, проценты готовности и имена "
            "инструментов — служебные детали, человеку они не нужны. "
            "Оценки вида «9/10» переводите в слова («сильно релевантно», "
            "«по касательной»). Раздел с главными фактами и числами статьи "
            "называйте «Ключевые тезисы», а не «Ключевые цифры». "
            "Единственная служебная деталь, нужная в ответе, — номера "
            "статей (#123): по ним человек находит источник."
        ),
    )

    @mcp.tool()
    async def search_articles(query: str, limit: int = 10) -> str:
        """Найти статьи в базе данных по смыслу запроса.
        ...
        Запрос лучше формулировать по-английски: база данных англоязычная.
        """
    ...
    async def suggest_ideas(
        ...
        У статьи бывают применения для нескольких компаний — выдача укажет,
        где искать остальные.
        """
        return await _call(
    ...
    async def company_brief(company: str) -> str:
        """Стартовый бриф по компании ОДНИМ вызовом: покрытие базы данных,
    ...
    async def corpus_coverage() -> str:
        """Карта базы данных: сколько статей, по каким компаниям, где дыры.

        Вызывайте ПЕРЕД глубоким поиском по узкой теме: если по компании или
        домену статей единицы — это дыра базы данных, и надо расширять ленты, а
    ...
    async def list_companies() -> str:
        """Компании, по которым размечена база данных, с числом релевантных статей."""
    ...
    @mcp.tool()
    async def prd_brief(article_id: int, stack: str = "") -> str:
        ...
        в базу данных можно из кабинета.

        ``stack`` — на чём делать демо: «React + TypeScript», «FastAPI +
        HTML», «Next.js». Передавайте то, что назвал человек. Если он стэк
        не назвал — СПРОСИТЕ, прежде чем писать: промпт вернётся с пометкой
        `NOT SPECIFIED`, а документ под чужой стэк выбрасывают целиком.
        """
        return await _call(lambda c: _prd_brief(c, article_id, stack))

    @mcp.tool()
    async def deck_brief(article_id: int) -> str:
        """Материалы для презентации, которую вы соберёте САМИ.

        Возвращает план слайдов (если он есть в базе данных) плюс выжимку и
...
async def _prd_brief(
    client: AiresClient, article_id: int, stack: str = ""
) -> str:
    brief = await client.prd_brief(article_id, stack)
...
async def _deck_brief(client: AiresClient, article_id: int) -> str:
    ...
    if plan:
        ...
        parts += [
            "## Плана слайдов в базе данных нет",
